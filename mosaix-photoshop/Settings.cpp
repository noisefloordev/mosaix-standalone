...
static void ReadFromKeys(GetKeys &keys, Mosaic::Options &options, bool bReadOptions)
{
...
static void WriteToKeys(PutKeys &keys, const Mosaic::Options &options, bool bOmitDefaults)
{
	const Mosaic::Options Defaults;
...
void ReadRegistryParameters(Mosaic::Options &options)
{
	if(gFilterRecord->sSPBasic == NULL)
		return;

	GetKeysRegistry keys;
	ReadFromKeys(keys, options, true);
}

void WriteRegistryParameters(const Mosaic::Options &options)
{
	if(gFilterRecord->sSPBasic == NULL)
		return;

	PutKeysRegistry keys;
	WriteToKeys(keys, options, false);
}

void ReadScriptParameters(Mosaic::Options &options)
{
	if(gFilterRecord->sSPBasic == NULL)
		return;
	...
	ReadFromKeys(keys, options, false);
}

void WriteScriptParameters(const Mosaic::Options &options)
{
	if(gFilterRecord->sSPBasic == NULL)
		return;

	PutKeysAction keys;
	WriteToKeys(keys, options, true);
}
